...
export interface LiveStruct {
    ...
    read(field: string): number | bigint | Uint8Array | Int8Array | Uint16Array | Int16Array | Uint32Array | Int32Array | Float32Array | Float64Array | BigUint64Array | BigInt64Array;
    write(field: string, value: number | bigint | Iterable<number | bigint>): void;
    destroy(): void;
    [key: string]: any;
}
...
export function struct(fields: Record<string, StructType>): StructDefinition {
    ...
    for (const [name, typeStr] of Object.entries(fields)) {
        ...
        // align offset for 8-byte types
        if (primitiveSize(baseType) === 8) {
            offset = (offset + 7) & ~7; // round up to multiple of 8
        }
        ...
        Make(heap: Heap, initial?: Record<string, number | bigint | Iterable<number | bigint>>, region?: Uint8Array & { ptr: number }): LiveStruct {
            ...
            const live: LiveStruct = new Proxy({
                ...
                read(field: string) {
                    ...
                    const baseType = info.arrayLength ? info.type.replace(/\[\d+\]$/, '') as PrimitiveType : info.type as PrimitiveType;
                ...
                write(field: string, value: number | bigint | Iterable<number | bigint>) {
                    ...
                    if (info.arrayLength) {
                        ...
                        for (let i = 0; i < info.arrayLength; i++) {
                            ...
                            if (baseType === 'u64') typed[0] = toBigIntSafe(arr[i], false);
                        ...
                        if (baseType === 'u64') typed[0] = toBigIntSafe(value as number | bigint, false);
                ...
                destroy() {
                    if (!destroyed) {
                        ...
                        throw new Error("Struct already destroyed");
                    }
                }
            }, {
                get(target, prop) {
